...
export default function EditarDepartamentoPage() {
  ...
  useEffect(() => {
    api.getDepartamento(Number(id))
      .then((d: { nombre: string; descripcion?: string }) => { setNombre(d.nombre); setDescripcion(d.descripcion ?? ""); })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.actualizarDepartamento(Number(id), { nombre: nombre.trim(), descripcion: descripcion.trim() });
      ...
      <div className="max-w-lg mx-auto space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black text-slate-800 dark:text-white">Editar Departamento</h1>
              <p className="text-xs text-slate-400">Modifica los datos del departamento</p>
            </div>
          </div>
        </div>
