...
import { Building2, ArrowLeft, Save, RefreshCw } from "lucide-react";
...
export default function NuevoDepartamentoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.crearDepartamento({ nombre: nombre.trim(), descripcion: descripcion.trim() });
      ...
      <div className="max-w-lg mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black text-slate-800 dark:text-white">Nuevo Departamento</h1>
              <p className="text-xs text-slate-400">Completa los datos del departamento</p>
            </div>
          </div>
        </div>
          ...
          <div className={card + " p-6 space-y-5"}>
            ...
            <div className="space-y-1.5">
              ...
              <input value={nombre} onChange={e => setNombre(e.target.value)}
                className={inp} placeholder="Ej. Operaciones, Administración…" />
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button type="submit" disabled={saving}
                className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white transition-opacity disabled:opacity-60"
