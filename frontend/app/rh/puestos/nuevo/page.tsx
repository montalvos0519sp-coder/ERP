...
export default function NuevoPuestoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.crearPuesto({ nombre: nombre.trim(), descripcion: descripcion.trim(), salario_base: salarioBase || null });
      ...
      <div className="max-w-lg mx-auto space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black text-slate-800 dark:text-white">Nuevo Puesto</h1>
              <p className="text-xs text-slate-400">Completa los datos del puesto</p>
            </div>
          </div>
        </div>
